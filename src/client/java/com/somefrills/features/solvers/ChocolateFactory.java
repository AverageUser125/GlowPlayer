...
public class ChocolateFactory {
    ...
    private static final Clock lastClaimClock = new Clock();
    ...
    public static void onHudTick(HudRenderEvent event) {
        ...
        for (Slot slot : handler.slots) {
            ...
            if (displayName.contains("CLICK ME!")) {
                // Check click delay
                if (!lastClaimClock.ended(claimDelay.value())) return;
                Utils.clickSlot(slot.getIndex());
                lastClaimClock.update();
            ...
            if (displayName.contains("Golden Rabbit")) {
                // Check click delay
                if (!lastClaimClock.ended(claimDelay.value() * 100L)) return;
                Utils.clickSlot(slot.getIndex());
                lastClaimClock.update();
                return; // Only click one per tick
            }
